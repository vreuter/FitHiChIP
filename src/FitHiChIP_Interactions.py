...
def main():
    ...
    parser.add_option("-p", "--peakfile", dest="peakfile",
                      help="Peak detection output file")
    parser.add_option("-M", "--Method", dest="Method", type="int", 
                      help="Type of interaction to be derived (1 - peak to peak, 2 - peak to non peak, 3 - all to all). Default 3.")
    parser.add_option("-l", "--alignfile", dest="alignfile",
                      help="Input alignment file (in BAM or PAIRIX format)")
    parser.add_option("-o", "--outfile", dest="outfile",
                      help="output bed filename storing the interactions")
    parser.add_option("-U", "--upperbound", dest="distUpThres", type="int",
                      help="OPTIONAL: upper bound on the intra-chromosomal distance range. DEFAULT 200000.")
    parser.add_option("-L", "--lowerbound", dest="distLowThres", type="int",
                      help="OPTIONAL: lower bound on the intra-chromosomal distance range. DEFAULT 20000.")
    parser.add_option("-t", "--threads", dest="threads",
                      help="Number of threads employed for computation. DEFAULT 1.")
    parser.add_option("-b", "--binsize", dest="binsize", type="int",
                      help="Size of bins employed. DEFAULT 5 Kb.")
    parser.add_option("-f", "--filefmt", dest="filefmt", type="int",
                      help="Input file format (1 = BAM file, 2 = pairix file. Default 1.")  
    parser.add_option("-c", "--coveragefile", dest="coveragefile",
                      help="output bed filename storing the coverage of individual genomic segments")
    ...
    if (input_file_fmt == 1):
        ...
        for _read in bam_input_file.fetch(anchor_chr, anchor_start, anchor_end):
            ...
            if curr_key in SegmentDict:
                ...
                if (abs(curr_pos_bin_start - anchor_start_bin) >= distLowThres) and (abs(curr_pos_bin_start - anchor_start_bin) <= distUpThres):
